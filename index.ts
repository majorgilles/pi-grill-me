...
const GITHUB_REPO_PERMISSION_GUIDANCE =
	"If approved GitHub issue output needs a repository and the current folder is not a git repo or has no usable remote, do not frame that as a refusal or downgrade to drafts by default. Treat it as a permission gate: ask the permission question directly, such as approving initialization of this folder, creating/publishing OWNER/REPO with GitHub CLI, adding the remote, and then creating the previewed issues; allow the user to provide a different OWNER/REPO/remote.";
